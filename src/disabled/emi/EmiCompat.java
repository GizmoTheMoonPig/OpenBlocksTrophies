...
public class EmiCompat implements EmiPlugin {
	public static final EmiRecipeCategory TROPHY = new EmiRecipeCategory(OpenBlocksTrophies.prefix("trophy"), EmiStack.of(TrophyItemHelper.createCyclingTrophy(EntityType.CHICKEN)));
			...
			for (Map.Entry<Identifier, Trophy> trophyEntry : Trophy.getTrophies().entrySet()) {
				...
				if (!trophyEntry.getValue().getVariants(Minecraft.getInstance().level.registryAccess()).isEmpty()) {
					for (CompoundTag tag : trophyEntry.getValue().getVariants(Minecraft.getInstance().level.registryAccess())) {
						registry.addRecipe(new EmiTrophyRecipe(Identifier.parse(tag.get(tag.getAllKeys().stream().toList().getFirst()).getAsString() + "_" + trophyEntry.getKey().toString().replace(':', '_')), trophyEntry.getValue(), tag));
					}
				} else {
