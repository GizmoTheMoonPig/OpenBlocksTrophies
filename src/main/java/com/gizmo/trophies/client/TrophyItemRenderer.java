	...
	public void renderByItem(ItemStack stack, ItemTransforms.TransformType type, PoseStack ms, MultiBufferSource source, int light, int overlay) {
		...
		if (item instanceof TrophyItem) {
			...
			if (stack.hasTag()) {
				...
				if (trophy != null) {
					TrophyRenderer.renderEntity(null, TrophyItem.getTrophyVariant(stack), stack.hasCustomHoverName() ? stack.getHoverName().getString() : "", Minecraft.getInstance().level, BlockPos.ZERO, trophy, ms, source, light);
				}
			}
		}
	}
