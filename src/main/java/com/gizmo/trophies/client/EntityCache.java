	...
	public static LivingEntity fetchEntity(EntityType<?> type, @Nullable Level level, CompoundTag variant, Optional<CompoundTag> defaultVariant) {
		if (level != null && !IGNORED_ENTITIES.contains(type)) {
			...
			if (entity instanceof LivingEntity living) {
				...
				if (!tag.isEmpty()) {
					try (ProblemReporter.ScopedCollector collector = new ProblemReporter.ScopedCollector(living.problemPath(), LOGGER)) {
						living.load(TagValueInput.create(collector, living.registryAccess(), tag));
					}
